...
- `src/product-search/` — the full hexagonal module, wired into `AppModule` alongside `ConfigModule.forRoot({ isGlobal: true })`:
  ...
  - Unit tests only so far (`*.spec.ts` beside each file) — e2e tests are a separate task, see below
...
Still missing: `.github/workflows/ci.yml`, e2e tests, and the Postman collection. Everything below this point describes the target design — don't assume a piece is implemented without checking. Update this section as pieces land.
